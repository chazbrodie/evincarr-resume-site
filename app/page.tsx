...
<main className="min-h-screen bg-gradient-to-br from-slate-700 via-slate-600 to-slate-700 relative overflow-hidden">      {/* Background decorative elements */}
      <div className="absolute top-0 left-0 w-96 h-96 bg-cyan-700/5 rounded-full blur-3xl"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl"></div>
      <div className="absolute top-1/2 left-1/4 w-64 h-64 bg-cyan-700/5 rounded-full blur-3xl"></div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-8 relative z-10">
        ...
        <header className="bg-gradient-to-br from-slate-800/90 to-slate-700/90 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-10 mb-6 md:mb-8 border border-slate-600/50 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-700/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
          <div className="relative flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
            {/* Photo */}
            <div className="flex-shrink-0 w-full md:w-auto flex justify-center md:justify-start">
              <div className="relative w-32 h-32 rounded-2xl overflow-hidden border-4 border-cyan-700/30 shadow-xl">
                <Image
                  src="/headshot.jpg"
                  alt="Evin Carr"
                  fill
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-xl md:text-2xl text-cyan-600 mb-4 font-medium">
                {resumeData.title}
              </p>
              <div className="flex flex-wrap justify-center md:justify-start gap-3 md:gap-4 text-slate-300 text-base">
                ...
                <a href={`mailto:${resumeData.email}`} className="flex items-center gap-1 text-cyan-600 hover:text-green-300 transition-colors">
  ✉️ {resumeData.email}
</a>
                <a href={`https://${resumeData.linkedin}`} target="_blank" className="text-cyan-600 hover:text-green-300 font-medium transition-colors flex items-center gap-1">
                  💼 LinkedIn
                </a>
                <a href="/Evin-Carr-Resume.pdf" download className="text-cyan-600 hover:text-green-300 font-medium transition-colors flex items-center gap-1">
                  📄 Resume PDF
                </a>
              </div>
            </div>
        ...
        {/* Quick Chat CTA */}
        {!chatOpen && (
<div className="bg-gradient-to-r from-cyan-700 to-cyan-600 rounded-2xl shadow-2xl p-6 md:p-8 mb-6 md:mb-8 text-white border border-cyan-700/30 relative overflow-hidden">            <div className="absolute top-0 left-0 w-32 h-32 bg-white/10 rounded-full -ml-16 -mt-16 blur-2xl"></div>
            <div className="flex flex-col md:flex-row items-center md:items-center justify-between gap-4 relative z-10">
              ...
              <button
                ...
                className="bg-white text-cyan-600 px-6 py-3 rounded-lg font-semibold hover:bg-slate-100 transition-all shadow-lg whitespace-nowrap w-full md:w-auto"
              >
                Start Chat
              </button>
            </div>
            ...
            <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto">
                      {suggestedQuestions.map((q, i) => (
                        <button
                          key={i}
                          onClick={() => setInput(q)}
                          className="text-left p-3 bg-slate-800/80 rounded-lg border border-slate-700 hover:border-cyan-700 hover:shadow-md transition-all text-sm text-slate-300"
                        >
                          💡 {q}
                        </button>
                      ))}
                  ...
                  >
                    <div
                      className={`inline-block p-3 md:p-4 rounded-xl max-w-[85%] md:max-w-[80%] text-sm md:text-base ${
                        msg.role === 'user'
                          ? 'bg-gradient-to-br from-cyan-700 to-cyan-600 text-white shadow-md'
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 p-3 md:p-4 bg-slate-900/50 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-700 focus:border-transparent transition-all text-white placeholder-slate-500 text-sm md:text-base"
                  disabled={isLoading}
                />
                <button
                  onClick={sendMessage}
                  disabled={isLoading || !input.trim()}
                  className="bg-gradient-to-br from-cyan-700 to-cyan-600 text-white px-6 md:px-8 py-3 md:py-4 rounded-xl hover:from-cyan-700 hover:to-blue-500 disabled:from-slate-700 disabled:to-slate-700 disabled:cursor-not-allowed font-semibold shadow-lg transition-all text-sm md:text-base"
                >
                  Send
                </button>
              </div>
        ...
        <section className="bg-slate-800/90 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-8 mb-6 md:mb-8 border border-slate-600/50 relative overflow-hidden">
          <div className="absolute bottom-0 right-0 w-48 h-48 bg-cyan-700/5 rounded-full -mr-24 -mb-24 blur-2xl"></div>
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
          {/* Automations */}
          <section className="bg-gradient-to-br from-cyan-700 to-cyan-600 rounded-2xl shadow-2xl p-6 md:p-8 text-white border border-cyan-700/30 relative overflow-hidden">
          ...
          {/* Achievements */}
          <section className="bg-slate-800/90 backdrop-blur border-2 border-cyan-700/50 rounded-2xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
            <div className="absolute bottom-0 left-0 w-32 h-32 bg-cyan-700/10 rounded-full -ml-16 -mb-16 blur-2xl"></div>
            <div className="relative">
              ...
              <ul className="space-y-2 md:space-y-3">
                {resumeData.highlights?.achievements?.map((item, i) => (
                  <li key={i} className="flex items-start text-xs md:text-sm text-slate-300 leading-relaxed">
                    <span className="mr-2 mt-0.5 text-cyan-600">→</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
        ...
        <section className="bg-slate-800/90 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-8 mb-6 md:mb-8 border border-slate-600/50">
          ...
          <div className="space-y-6 md:space-y-8">
            {resumeData.experience?.map((job, index) => (
              <div key={index} className="border-l-4 border-cyan-700 pl-4 md:pl-6 py-2 hover:border-cyan-600 transition-all">
                <h3 className="text-lg md:text-xl font-bold text-white">
                  {job.role}
                </h3>
                <p className="text-base md:text-lg text-cyan-600 font-semibold mb-1">
                  {job.company}
                </p>
        ...
        {resumeData.education && (
          <section className="bg-slate-800/90 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-8 mb-6 md:mb-8 border border-slate-600/50">
            ...
            <div className="border-l-4 border-cyan-700 pl-4 md:pl-6">
              <h3 className="text-lg md:text-xl font-bold text-white">
        ...
        <section className="bg-slate-800/90 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-8 mb-6 md:mb-8 border border-slate-600/50">
          ...
          <div className="grid md:grid-cols-2 gap-4 md:gap-6">
            {/* AI & Automation */}
            <div>
              <h3 className="text-base md:text-lg font-bold text-cyan-600 mb-3 flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {resumeData.skills?.aiAutomation?.map((skill, index) => (
                  <span
                    key={index}
                    className="bg-slate-900/50 text-slate-300 px-3 py-1.5 rounded-lg text-xs md:text-sm font-medium border border-slate-700 hover:border-cyan-700 hover:shadow-lg transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
        ...
        <footer className="text-center text-slate-400 text-sm py-8 border-t border-slate-700/50 mt-8">
          ...
          <p className="text-slate-500">
            Last updated: February 2026 • <a href={`mailto:${resumeData.email}`} className="text-cyan-600 hover:text-green-300 transition-colors">Get in touch</a>
          </p>
        </footer>
      </div>
    </main>
